...
const ENV_CONSUMER_KEY: &str = "HATENA_CONSUMER_KEY";
const ENV_CONSUMER_SECRET: &str = "HATENA_CONSUMER_SECRET";
const ENV_OAUTH_VERIFIER: &str = "HATENA_OAUTH_VERIFIER";
...
impl RequestToken {
  ...
  fn get_signature(&self, url: &str, params: &HashMap<&str, &str>) -> String {
    let key = format!(
      "{}&{}",
      self.encode(&self.consumer_secret),
      self.encode(&self.oauth_token_secret.clone().unwrap_or("".into()))
    );
    ...
    let base_string = format!(
      ...
      self.encode("POST"),
      self.encode(url),
      self.encode(&sorted_params.join("&")),
    );
...
pub fn get_request_token() -> Result<OauthTokenResponse, OauthError> {
  ...
  let res = client
    ...
    .body(format!(
      "scope={}",
      percent_encode(&scopes.join("&").as_bytes(), STRICT_ENCODE_SET)
    ))
    .send();
